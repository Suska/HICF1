...
Import clinical data, clean up clinical data and genetic factor data and merge both tables

```{r}
setwd("~/work/HICF1/HICF1_sub1/trunk/HICF_predictivemodel")

#Import factor data
genf <- as.data.frame(read.table("HICF1_genfactors.csv", sep="\t", header=TRUE, quote=""))
colnames(genf)[1] <- "ID"

#import clinical data
clindat <- as.data.frame(read.table("clinicaldata_02052014.csv", sep="\t", header=TRUE, quote=""))
clindat[clindat==""]<-NA
colnames(clindat)[1] <- "ID"

#clean up clinical data
colnames(clindat)[3] <- "MRD"


#import subclone data
clones <- as.data.frame(read.table("HICF1_clones.csv", sep="\t", header=TRUE, quote="",na.strings=c("", "NA")))

clones[clones==""]<- NA
colnames(clones)[1] <- "ID"

genclin <- merge(clindat, genf, by="ID")
genclin <- merge(genclin, clones, by="ID")
```


Total number of CNAs and MRD
...
ggplot(data=CNAfreq, aes(x=factor(CNAfreq$Total.numer.of.CNAs), y=CNAfreq$Frequency, fill=factor(CNAfreq$MRD)) + geom_bar(stat="identity")

ggplot(diamonds, aes(CNAfreq, fill=CNAfreq$MRD)) + geom_bar(position="dodge")
       
ggplot(CNAfreq,aes(x=factor(Year),y=MWh,fill=factor(Metric))) + geom_bar(position="stack")
...
# Building the table and doing CHi-square test
mut_M <- as.table(rbind(c(110, 0), c(92, 15)))
dimnames(mut_M) <- list(MRD = c("MRD_negativ","MRD_positive"),
                    TP53_status = c("TP53_not_mut","TP53_mut"))
(Xsq <- chisq.test(mut_M))  # Prints test summary
