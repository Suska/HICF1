...
#genclinv6$TP53_lowVAF <- genclinv6$TP53_mut_All_VAFs-genclinv6$TP53_morethan5VAF
genclinv6[,c(2:43, 46)] <- lapply(genclinv6[,c(2:43, 46)], as.factor)
...
univariate_variables <- c("TP53_ALL", "TP53_mut", "TP53_bi", "ATM_ALL", "ATM_bi", "ATM_del", "ATM_mono", "BIRC3_ALL", "BIRC3_bi", "BIRC3_del", "BIRC3_mono", "SAMHD1_ALL", "Trisomy_12", "NOTCH1_mut", "SF3B1_mut", "Subclones", "Total_num_CNAs", "vh_mutation_status", "age_at_randomisation", "patient_gender", "Binet", "WBC", "cd38", "MRD")

genclinv6.univar <- genclinv6[colnames(genclinv6) %in% univariate_variables]
genclinv6.univar.pvalues <- lapply(genclinv6.univar[,-22], univariate.pvalue.genclinv6)
...
genclinv6.univar.pvalues$corrected.p.value <- p.adjust(genclinv6.univar.pvalues$p.value, method="holm") 
...
genclinv6.univar.pvalues$MRDpos0 <- as.integer(unlist(lapply(genclinv6.univar[,-22], univariate.n.MRDpos.0)))
genclinv6.univar.pvalues$MRDneg0 <- as.integer(unlist(lapply(genclinv6.univar[,-22], univariate.n.MRDneg.0)))
genclinv6.univar.pvalues$MRDpos1 <- as.integer(unlist(lapply(genclinv6.univar[,-22], univariate.n.MRDpos.1)))
genclinv6.univar.pvalues$MRDneg1 <- as.integer(unlist(lapply(genclinv6.univar[,-22], univariate.n.MRDneg.1)))
...
genclinv6.univar.pvalues$testused <- unlist(lapply(genclinv6.univar[,-22], univariate.testused))
...
variables_ass <- c("TP53_ALL", "TP53_mut", "TP53_del", "TP53_cnLOH", "ATM_ALL", "ATM_mut", "ATM_del", "ATM_cnLOH", "BIRC3_ALL", "BIRC3_mut", "BIRC3_del", "NOTCH1_mut", "SF3B1_mut", "X13q_ALL", "X6q._del_ALL", "Trisomy_12", "Trisomy_18", "Trisomy_19", "XPO1_gain", "SAMHD1_ALL", "MYD88_mut", "MED12mutation", "X8q_ALL", "Subclones", "Total_num_CNAs", "Binet", "age_at_randomisation", "patient_gender", "vh_mutation_status", "WBC", "cd38")
...
genclinv6.ass$age_at_randomisation <- as.numeric(genclinv6.ass$age_at_randomisation)
for (i in 1:31){
  for (j in 1:31){
...
ass.pvalues[,c(2:32)] <- lapply(ass.pvalues[,c(2:32)], as.numeric)

ass.pvalues.corrected <- as.matrix(ass.pvalues[, c(2:32)]) 
...
ass.pvalues[,c(2:32)] <- round(ass.pvalues[,c(2:32)], digits=3)
...
sigstars <- apply(ass.pvalues.corrected[2:32], c(1,2), significancelevels) 
...
stargazer(fit.sum.gen1,fit.sum.gen2, summary=FALSE, title="Multiple log regression, n=209", font.size="tiny", single.row=TRUE, column.labels=c("genetic 1", "genetic2"), model.numbers=FALSE, notes.align="l", digits=2)
@
%\end{landscape}
