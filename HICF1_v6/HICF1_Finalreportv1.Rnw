...
genclinv6.pvalues <- lapply(genclinv6[,-c(49)], univariate.pvalue.genclinv6)
genclinv6.pvalues <- unlist(genclinv6.pvalues)
genclinv6.pvalues <- as.data.frame(genclinv6.pvalues)
colnames(genclinv6.pvalues) <- "p.value"
#This rounds the p-value
genclinv6.pvalues$p.value <- signif(genclinv6.pvalues$p.value, 3)
#This corrects the p-value
genclinv6.pvalues$uncorrected <- mapply(significance, genclinv6.pvalues$p.value)
genclinv6.pvalues$corrected.p.value <- p.adjust(genclinv6.pvalues$p.value, method="holm") 

#This gives the significant levels
genclinv6.pvalues$corrected <- mapply(significance, genclinv6.pvalues$corrected.p.value)

#This gives the n in each group
genclinv6.pvalues$MRDpos0 <- as.integer(unlist(lapply(genclinv6[,-c(49)], univariate.n.MRDpos.0)))
genclinv6.pvalues$MRDneg0 <- as.integer(unlist(lapply(genclinv6[,-c(49)], univariate.n.MRDneg.0)))
genclinv6.pvalues$MRDpos1 <- as.integer(unlist(lapply(genclinv6[,-c(49)], univariate.n.MRDpos.1)))
genclinv6.pvalues$MRDneg1 <- as.integer(unlist(lapply(genclinv6[,-c(49)], univariate.n.MRDneg.1)))
genclinv6.pvalues$sum <- genclinv6.pvalues$MRDpos0+genclinv6.pvalues$MRDneg0+genclinv6.pvalues$MRDpos1+genclinv6.pvalues$MRDneg1
#This gives the test that was used:
genclinv6.pvalues$testused <- unlist(lapply(genclinv6[,-c(49)], univariate.testused))

# write.csv(genclinv6.pvalues, file="/home/andreas/suska/work/01_HICF1/HICF1_sub1/trunk/HICF1_v6/Univariatepvalues_SW_17072014.csv", sep="\t")
@
