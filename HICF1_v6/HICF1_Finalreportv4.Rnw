...
univariate_variables <- c("TP53_ALL", "TP53_mut", "TP53_bi", "ATM_ALL", "ATM_bi", "ATM_del", "ATM_mono", "BIRC3_ALL", "BIRC3_bi", "BIRC3_del", "BIRC3_mono", "SAMHD1_ALL", "Trisomy_12", "NOTCH1_mut", "SF3B1_mut", "Subclones", "Total_num_CNAs", "MRD", "X11q_mono", "TP53_lowVAF")
...
genclinv6.univar.pvalues$MRDpos_0 <- (round(as.integer(unlist(lapply(genclinv6.univar[,-19], univariate.n.MRDpos.0)))/209,2))*100
genclinv6.univar.pvalues$MRDpos_0 <- paste(genclinv6.univar.pvalues$MRDpos_0, "%", sep="")
genclinv6.univar.pvalues$MRDneg_0 <- round(as.integer(unlist(lapply(genclinv6.univar[,-19], univariate.n.MRDneg.0)))/209, 2)*100
genclinv6.univar.pvalues$MRDneg_0 <- paste(genclinv6.univar.pvalues$MRDneg_0, "%", sep="")
genclinv6.univar.pvalues$MRDpos_1 <- round(as.integer(unlist(lapply(genclinv6.univar[,-19], univariate.n.MRDpos.1)))/209,2)*100
genclinv6.univar.pvalues$MRDpos_1 <- paste(genclinv6.univar.pvalues$MRDpos_1, "%", sep="")
genclinv6.univar.pvalues$MRDneg_1 <- round(as.integer(unlist(lapply(genclinv6.univar[,-19], univariate.n.MRDneg.1)))/209,2)*100
...
genclinv6.univar.pvalues <- genclinv6.univar.pvalues[c(1:11, 14:19, 12, 13),]
...
align(pvaluetable) <- "|r|r|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|"
...
align(corr.pvaluetable) <- "|r|r|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|"
...
align(oddsratiotable) <- "|r|r|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|"
...
#RESULT TABLE in a nice format!
library(stargazer)
stargazer(fit.sum.gen1,fit.sum.gen2, fit.sum.gen3,fit.sum.gen4,fit.sum.gen5, fit.vhmut, fit.Binet, summary=FALSE, title="Multiple log regression, n=209", font.size="tiny", single.row=TRUE, column.labels=c("genetic1", "genetic2", "genetic3", "genetic4", "genetic5", "vhmut", "Binet"), model.numbers=FALSE, notes.align="l", digits=2)
@
...
missclasserror$model <- c("genetic1", "genetic2", "genetic3", "genetic4", "genetic5", "Binet", "vhmutation")
...
FalseNegativeRate.rf[i] <- confusion[4]/(confusion[1]+confusion[2])
#Probabilit that someone is predicted negative(MRD+), given that someone is negative
TrueNegativeRate.rf[i] <- confusion[2]/(confusion[3]+confusion[4])
}
...
#Probability that someone is predicted negative(MRD+), given that someone is positive(MRD-)
FalseNegativeRate.logreg[i] <- confusion[4]/(confusion[1]+confusion[2])
#Probabilit that someone is predicted negative(MRD+), given that someone is negative
TrueNegativeRate.logreg[i] <- confusion[2]/(confusion[3]+confusion[4])
}
...
#Probability that someone is predicted negative(MRD+), given that someone is positive(MRD-)
FalseNegativeRate.vhmut[i] <- confusion[4]/(confusion[1]+confusion[2])
#Probabilit that someone is predicted negative(MRD+), given that someone is negative
TrueNegativeRate.vhmut[i] <- confusion[2]/(confusion[3]+confusion[4])
}
 
@
 
<<ROC,echo=FALSE, width=9>>=
ROC.data <- as.data.frame(cbind(FalsePositiveRate.logreg, TruePositiveRate.logreg, TrueNegativeRate.logreg, FalseNegativeRate.logreg, FalsePositiveRate.rf, TruePositiveRate.logreg, TrueNegativeRate.rf, FalseNegativeRate.rf, FalsePositiveRate.vhmut, TruePositiveRate.vhmut, TrueNegativeRate.vhmut, FalseNegativeRate.vhmut))
# 
# png("ROC.png")
# source("/home/andreas/suska/work/X_usefulcode/Rfunctions/multiplotfunction.R")
# 
# Positives <- ggplot(data=ROC.data)+geom_line(aes(x=FalsePositiveRate.logreg, y=TruePositiveRate.logreg), colour="blue")+
#   geom_line(aes(x=FalsePositiveRate.rf, y=TruePositiveRate.rf), colour="red")+
#   geom_line(aes(x=FalsePositiveRate.vhmut, y=TruePositiveRate.vhmut), colour="green")+
...
#   geom_line(aes(x=FalseNegativeRate.vhmut, y=TrueNegativeRate.vhmut), colour="green")+
#   theme_bw(base_size = 12, base_family = "")+geom_abline(intercept=0, slope=1, linetype=3, alpha=0.5)+
#   xlim(c(0,1))+
